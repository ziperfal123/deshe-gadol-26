...
- **special_predictions.json** — 153 objects, one per player, already deduped. Fields: `top_scorer, best_player, top_assists, most_goals_group_stage_team, most_conceded_group_stage_team, most_goals_tournament_team, most_conceded_tournament_team, most_cards_team, least_cards_team, total_red_cards, total_extra_time, total_penalties`.
  - ✅ **Name spellings canonicalized** (`scripts/canonicalize_specials.py`): free-text player/team names were collapsed to one spelling per player/team (e.g. "מסי / לאו מסי / ליאו מסי" → "ליאו מסי"; "אמבפה / קליאן מבאפה" → "קיליאן אמבפה"). 116 → 45 distinct players; team names normalized to `teams.json` `name_he`. **Only the spelling was changed** — no pick/number altered. Re-runnable (idempotent); rerun if the seed is re-extracted, then regenerate `data/public`.
...
## 5c. Qualification scoring + special-bet overrides

- **Qualification (group→knockout):** `data/seed/qualification.json` (built from ESPN standings; FIFA's site has no fetchable data endpoint) holds the 32 qualifiers and each one's final group rank. The engine scores group-stage advancement picks **+2 if the team qualified (any place) + 1 bonus if the predicted position equals the actual rank**, added to the **official** total. Player page shows green/red chips + a floating "+1" bonus badge + a lean summary bar; per-player `qualification` block in the player JSON.
- **Special-bet hardcoded fallback:** `data/seed/special_overrides.json`. Priority is **live feed/API first** (fresher); a hardcoded field is used **only when that field has no live data** (e.g. cards/assists with no API key). Fields with neither show "לא זמין". Team fields = FIFA codes, player fields = canonical Hebrew names; ties list all, every picker scored. The engine builds per-field `lead_set`/`lead_val`/`live` dicts and applies the fallback only when `live[key]` is false; `leaders.json` marks each field `source: override|computed`. (So enabling the API-Football key later automatically takes over cards/assists from the hardcoded values.)
